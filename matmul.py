...
def MatMul(mat_a, mat_b, mat_out):
    ...
    mat_b = mat_b.reshape(shape_n, shape_k)
        ...
        for n_index in range(shape_n):
            for k_index in range(shape_k):
                mat_out[m_index, n_index] += mat_a[m_index, k_index] \
                                           * mat_b[n_index, k_index]
    # print(mat_a.shape, mat_b.shape, mat_out.shape, mat_out)
    return None
...
def MatMul_SnSn(mat_a, mat_b, mat_out):
    ...
    if shape_m >= shape_n:
        ...
        for index in range(shape_n):
            mat_out_offset[0, index] = sum(mat_b[:, index])
...
def matmul_test(test_case, bits):
    ...
    # print('mat_out_ref', mat_out_ref.dtype, mat_out_ref)
    # print('mat_out', mat_out.dtype, mat_out)
